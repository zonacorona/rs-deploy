...
<%
    ...
    */
    ...
    //If userName has no value, we check to see if debuggin is enabled, if so render the page without
    //any x-ldap-username value and show all the deployable wars
    if(userName==null||userName.isEmpty()){
		...
		if(debug.equalsIgnoreCase("false")){
			...
			//Otherwise, do nothing and allow ExternalWarController to render the page
		}
    }    
        ...
        <form id="filterformid" name="filterform">
          <span id="filterspan" class="filterspan">    
            <select name="docbooks" id="docbooks" multiple size="6">
              ...
              <c:forEach var="adocbook" items="${extFilters}" varStatus="status">
...
<%-- End of div id="top" --%>       
    ...
    <c:choose>
      ...
      <c:otherwise>    
        <table class="deploytable" id="deploytable">
            ...
            --%>           
              <tr class="deployrow" id="deployrow0">
                ...
                <td class="deploywarcol">
                  <ul>
                    <li>
                    <input type="checkbox" class="warfoldernames external" name="warfoldernames" value="ROOT" title="ROOT" >
                ...
                <td class="detailscol">
                    ...
                    <input type="image" src="<c:url value='/resources/images/magnifyingglass.jpeg'/>" class="detailsimage external" id="detailsimageid0" title="Click for detailed information"
                      ng-click="handleDetails($event);"/>
                    <%--
                    <img src"<c:url value='/resources/images/magnifyingglass.jpeg'>" class="detailsimage external" id="detailsimageid0" title="Click for detailed information"
                      style="width:20px;"/>
                    --%>               
                ...
                <td class="deploybuttoncol">             
                  <input style="background-color:rgb(95,170,259);" id="deploybutton0" type="button" class="deploybutton external" name="deploybutton" value="Deploy to Production"  
                    ng-click="deployToProd('deployrow0','com.rackspace.cloud.api~~~rax-indexwar','0','false');"
                  ...
                  <p>
                    <input style="background-color:rgb(204,102,51);" id="revertbutton0" type="button" class="revertbutton external" name="revertbutton" value="Revert to Backup"  
                      ...
                      'com.rackspace.cloud.api~~~rax-indexwar','0', 'false')"
         ...
         <c:forEach var="awar" items="${extWars}" varStatus="status">
             <tr class="deployrow" id="deployrow${(status.index+1)}" >
                     ...
                     <c:forEach var="adocnamenfolder" items="${awar.value.docNameNFolderNamesList}">              
                      ...
                      <c:choose> 
                          <c:when test="${fn:endsWith(adocnamenfolder.folderName,'internal')}">
                              <%-- 
                              <span style="color:#A2A2A2;position:relative;left:-30px;" title="<c:out value="${adocnamenfolder.folderName}"/>"><c:out value="${adocnamenfolder.docName}" /></span>
                              --%>
                          ...
                          <c:otherwise>
                              ...
                              <c:choose>    
                                  ...
                                  <c:otherwise>
                                      <span style="position:relative;left:-30px;margin:5px 5px 5px 33px;" title="<c:out value='${adocnamenfolder.folderName}'/>"><c:out value="${adocnamenfolder.docName}"/></span>
                                      cannot be deployed. It does not depend on the <a href="https://github.com/rackerlabs/clouddocs-maven-plugin-docbook">clouddocs-maven-plugin-docbook plugin.</a>
                 ...
                 <td class="detailscol">
                     ...
                     <input type="image" src="<c:url value='/resources/images/magnifyingglass.jpeg'/>" class="detailsimage external" id="detailsimageid${status.index+1}" title="Click for detailed information"
                     ng-click="handleDetails($event);" />
                     <%-- 
                     <img src="<c:url value='/resources/images/magnifyingglass.jpeg'/>" class="detailsimage external" id="detailsimageid${status.index+1}" title="Click for detailed information"
                     style="width:20px;"/>
                     --%>                  
    ...
    <c:otherwise>
      <div class="wrongserver">
        This page should only be used on the EXTERNAL STAGING server. If you need to deploy externally, go to: <a href="http://docs-staging.feedsso.rackspace.com/rax-autodeploy/DeployWars">Docs External Deploy</a>
      </div>    
    </c:otherwise>
  </c:choose>
...
<div id="changepasswordid" 
     ng-show="changepasswordwidget" 
